...
observation_time = datetime.datetime(2025, 5, 29, 15, 19, 28, 914876)

GHA_of_body_at_hour = (45 + 38.1 /60) # GHA of Sun on the hour from Almanac, increment is calculated
...
mySextant.Hs = 50 + 58.1 /60 # uncorrected sextant reading
mySextant.index_error= +2.2 # (On - / Off +) 
mySextant.eye_height = 2.5 # Sandvita: Cockpit = 2.5 Mast = 3.3
mySextant.semi_diameter = +15.8 # Varies, get from Almanac.
...
infoTable.append([f"Observation time {observation_time} UTC","",str(observation_time)])
...
outputTable.append([f"GHA Sun {observation_time.year}-{observation_time.month}-{observation_time.day} {observation_time.hour}h (Almanac)",f"{deg_to_dm(GHA_of_body_at_hour%360)}", GHA_of_body_at_hour])
...
for row in infoTable:
	print(row[0].ljust(column1_width), row[1].rjust(column2_width))
